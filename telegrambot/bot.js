...
async function linkGroupToTelegram(groupId, chatId) {
  try {
    ...
    log.success(`Группа ${group.groupName} успешно привязана к Telegram чату ${chatId}`);
...
async function createAndLinkNewGroup(chatId, chatTitle) {
  try {
    ...
    if (existingGroup) {
      ...
      if (existingGroup.telegramId && existingGroup.telegramId !== chatId.toString()) {
        return {
          success: false,
    ...
    log.success(`Создана новая группа: ${chatTitle} и привязана к Telegram чату ${chatId}`);
...
bot.on('callback_query', async (callbackQuery) => {
  ...
  if (data === 'attach_group') {
    ...
    let messageText = groups.length > 0 
...
async function sendExamNotification(exam) {
  try {
    ...
    log.success(`Уведомление о экзамене отправлено в группу ${group.groupName}`);
