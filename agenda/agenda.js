...
// Конфигурация
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/orbitest';
...
// Логирование
const log = {
  info: (msg) => console.log(`[AGENDA INFO] ${new Date().toISOString()} - ${msg}`),
  error: (msg, err) => console.error(`[AGENDA ERROR] ${new Date().toISOString()} - ${msg}`, err),
  success: (msg) => console.log(`[AGENDA SUCCESS] ${new Date().toISOString()} - ${msg}`),
  warn: (msg) => console.warn(`[AGENDA WARN] ${new Date().toISOString()} - ${msg}`)
};
...
async function sendReminderToGroup(exam, group, timeRemaining) {
  try {
    ...
    log.success(`Напоминание отправлено в группу ${group.groupName} для экзамена ${examName}`);
    return { success: true };

  } catch (error) {
    log.error(`Ошибка отправки напоминания в группу ${group.groupName}:`, error);
      ...
      try {
        ...
        if (reminderCheck.send) {
          ...
          if (group) {
            ...
            const result = await sendReminderToGroup(exam, group, timeRemaining);
...
async function startAgenda() {
  try {
    ...
    log.info(`Задача проверки экзаменов запланирована каждые ${CHECK_INTERVAL}`);
